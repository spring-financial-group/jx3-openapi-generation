NAME := jx3-openapi-generation
BINARY_NAME := jx3-openapi-generation
BUILD_TARGET = build
...
BUILDFLAGS :=  -ldflags \
  " -X $(ROOT_PACKAGE)/pkg/cmd/version.Version=$(VERSION)\
		-X github.com/spring-financial-group/jx3-package-generation/pkg/cmd/version.Version=$(VERSION)\
...
build: $(GO_DEPENDENCIES) clean ## Build jx3-openapi-generation binary for current OS
