...
echo Generating API
npx openapi-generator-cli generate -i ./spec.json -g java -o java_service --git-user-id $repoOwner --git-repo-id $repoId --global-property models,modelTests=false,modelDocs=false -p basePackage=$name -p modelPackage=$name -p dateLibrary=java8-localdatetime
